...
def predict_and_test(envs, path_to_data, forecast_module, idx, flag='predict'):
    ...
    if ".zip" in path_to_data:
        ...
        items = os.listdir(path_to_data)
        files = sorted(items)
        path_to_test_file = os.path.join(path_to_data, files[idx])
        return exec_predict_and_test(envs, path_to_test_file, forecast_module, flag)
...
if __name__ == "__main__":
    ...
    NUM_MAX_RUNS = 1            # 最小值可以取1，最大值为你的测试目录下文件总数，eg: final_phase_test/infile下共有142个文件，则max=142
    MAX_TIMEOUT = 3600 * 10     # 10 hours
    MIN_TIME = 3                # 3 secs
    MIN_NOISE_LEVEL = 0.05      # 5 %
    path_code_test = "tests/submit"
    if ".zip" not in path_code_test:
        evaluate(path_code_test)
    else:
        eval(path_code_test)